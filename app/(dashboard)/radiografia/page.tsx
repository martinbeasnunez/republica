...
export default function RadiografiaIndexPage() {
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Analisis profundo de patrimonio, historial legal, redes de contactos y financiamiento de campana
        </p>
      </motion.div>
      ...
      <div className="flex items-start gap-3 rounded-xl border border-primary/20 bg-primary/5 p-4">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Datos basados en declaraciones juradas ante el JNE, registros del Poder Judicial,
            ONPE y fuentes oficiales de acceso publico. Los puntajes de riesgo son indicadores
            generados algoritmicamente y no constituyen juicio legal.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {candidatesWithData.map(({ candidate, radiografia }, index) => {
          ...
          const activeProcs = radiografia.legalHistory.filter(
            (l) => l.status === "activo" || l.status === "investigacion"
          ).length;
